...
const HeroServices = () => {
  ...
  const items = Array.isArray(data?.service_highlights)
    ? data.service_highlights
    : [];
  ...
  return (
    <section className="bg-myGreen-200">
      <div className="container p-4 lg:py-8 flex flex-col xl:flex-row-reverse items-start justify-between gap-4 xl:gap-20">
        ...
        >
          <h3 className="text-3xl lg:text-4xl mb-2 lg:mb-4">{mainTitle}</h3>
          <p>{description}</p>
        </motion.div>
        ...
        <motion.div
          ...
          whileInView="visible"
          viewport={{ once: true, amount: 0.3 }}
          className="flex flex-wrap gap-4 lg:gap-16 w-full"
        >
          {items.map((item) => (
            <motion.div
              key={item.id}
              variants={card}
              className="flex-1 min-w-44 p-4 rounded-md bg-myYellow-200 even:bg-myOrange-200 even:text-white text-black flex flex-col items-center gap-2 even:xl:-translate-y-20"
            >
